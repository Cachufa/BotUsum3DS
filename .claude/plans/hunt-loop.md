...
Happy path in the umbrella. Fail: soft reset, no in-game save. Miss (`sv=-1`): extra L+R+Start. Do **not** relaunch Azahar on miss (unlike Jirachi killing Dolphin). Title → Continue is the next 31s mash.
...
- [x] One attempt: sequence then RAM SV (`run_poipole_once`). Save is plan 08.
- [x] Default command = loop.
- [x] Log every attempt.
